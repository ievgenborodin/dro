define(function() {
    ...
    defHeight: 400,
    patterns: [],
    lr:80, lg:80, lb:80, la:130, 
    rr:100, rg:100, rb:100, ra:130,
    ...
    loadFile: function(config){
        ...
        that.lr = config.lr || that.lr;
        that.lg = config.lg || that.lg;
        that.lb = config.lb || that.lb;
        that.la = config.la || that.la;
        that.rr = config.rr || that.rr;
        that.rg = config.rg || that.rg;
        that.rb = config.rb || that.rb;
        that.ra = config.ra || that.ra;
        that.patterns = [{
            r: that.lr,
            g: that.lg,
            b: that.lb,
    ...
    setSize: function(image, screenWidth, screenHeight){
        var w, h, 
            ...
            ih = image.height,
            imageK = iw / ih,
            sw = Math.floor(screenWidth * 0.99),
            sh = Math.floor(screenHeight * 0.99);      
        if (ih > sh || iw > sw) {
            if (imageK > 1) {
                w = iw = sw;
    ...
    setDots: function(screen, idata, pattern){
            ...
            for (var i=0, j=0; i<=d.length; i+=4, j++) {
              if (((d[i]<p[0].r && d[i+1]<p[0].g && d[i+2]<p[0].b) || d[i+3]<p[0].a) || ((d[i]>p[1].r && d[i+1]>p[1].g && d[i+2]>p[1].b) || d[i+3]<p[1].a))
